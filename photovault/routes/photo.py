...
def enhance_photo_api(photo_id):
    ...
    try:
        ...
        from photovault.utils.enhanced_file_handler import get_image_info_enhanced
        enhanced_image_info = get_image_info_enhanced(enhanced_filepath)
        
        # Get file size with fallback
        file_size = None
        if enhanced_image_info:
            file_size = enhanced_image_info.get('size_bytes')
        if not file_size:
            try:
                if os.path.exists(enhanced_filepath):
                    file_size = os.path.getsize(enhanced_filepath)
                elif os.path.exists(temp_enhanced_filepath):
                    file_size = os.path.getsize(temp_enhanced_filepath)
                else:
                    file_size = getattr(photo, 'file_size', 0)
            except:
                file_size = getattr(photo, 'file_size', 0)
        
        # Create a NEW photo record for the enhanced image
        enhanced_photo = Photo()
        enhanced_photo.user_id = current_user.id
        enhanced_photo.filename = enhanced_filename
        enhanced_photo.original_name = f"Enhanced - {photo.original_name}"
        enhanced_photo.file_path = enhanced_filepath
        enhanced_photo.thumbnail_path = thumbnail_path
        enhanced_photo.file_size = file_size
        enhanced_photo.width = enhanced_image_info.get('width') if enhanced_image_info else getattr(photo, 'width', None)
        enhanced_photo.height = enhanced_image_info.get('height') if enhanced_image_info else getattr(photo, 'height', None)
        enhanced_photo.mime_type = enhanced_image_info.get('mime_type', 'image/jpeg') if enhanced_image_info else 'image/jpeg'
        enhanced_photo.upload_source = getattr(photo, 'upload_source', 'file')
        enhanced_photo.auto_enhanced = True
        enhanced_photo.enhancement_settings = str(applied_settings)
        
        # Copy ALL metadata from original photo (with safe attribute access)
        enhanced_photo.date_taken = getattr(photo, 'date_taken', None)
        enhanced_photo.camera_make = getattr(photo, 'camera_make', None)
        enhanced_photo.camera_model = getattr(photo, 'camera_model', None)
        enhanced_photo.iso = getattr(photo, 'iso', None)
        enhanced_photo.aperture = getattr(photo, 'aperture', None)
        enhanced_photo.shutter_speed = getattr(photo, 'shutter_speed', None)
        enhanced_photo.focal_length = getattr(photo, 'focal_length', None)
        enhanced_photo.flash_used = getattr(photo, 'flash_used', None)
        enhanced_photo.gps_latitude = getattr(photo, 'gps_latitude', None)
        enhanced_photo.gps_longitude = getattr(photo, 'gps_longitude', None)
        enhanced_photo.gps_altitude = getattr(photo, 'gps_altitude', None)
        enhanced_photo.location_name = getattr(photo, 'location_name', None)
        enhanced_photo.orientation = getattr(photo, 'orientation', None)
        enhanced_photo.color_space = getattr(photo, 'color_space', None)
        enhanced_photo.description = getattr(photo, 'description', None)
        enhanced_photo.tags = getattr(photo, 'tags', None)
        enhanced_photo.event_name = getattr(photo, 'event_name', None)
        enhanced_photo.estimated_year = getattr(photo, 'estimated_year', None)
        enhanced_photo.ai_metadata = getattr(photo, 'ai_metadata', None)
        
        # Also update original photo with enhanced version info for backward compatibility
        ...
        # Ensure database transaction is committed properly
        try:
            db.session.add(enhanced_photo)
            db.session.commit()
            # Verify the insert was successful by refreshing from database
            db.session.refresh(enhanced_photo)
            logger.info(f"Enhanced photo created as new record: {enhanced_photo.id}, filename: {enhanced_photo.filename}")
        except Exception as commit_error:
        ...
        logger.info(f"Successfully enhanced photo for user {current_user.id}, photo {photo_id}, new photo ID: {enhanced_photo.id}")
        
        return jsonify({
            'success': True,
            'message': 'Photo enhanced successfully',
            'photo_id': enhanced_photo.id,
...
def sharpen_photo_api(current_user, photo_id):
    """
    API endpoint to sharpen an image - Supports both session and JWT auth
    """
    try:
        from photovault.models import Photo
        from photovault.utils.image_enhancement import sharpen_image
        ...
        # Sharpened image path (temp local path)
        temp_sharpened_filepath = os.path.join(user_upload_dir, sharpened_filename)
        
        # Apply sharpening (saves to temp local)
        output_path, applied_settings = sharpen_image(
            full_file_path, 
        ...
        if app_storage.is_available():
            with open(temp_sharpened_filepath, 'rb') as f:
                ...
                if success:
                    ...
                    logger.info(f"Sharpened photo uploaded to App Storage: {storage_path}")
                    try:
        ...
        # Create thumbnail for sharpened version
        thumbnail_filename = f"{safe_username}.sharpened.{date}.{random_number}_thumb.jpg"
        if app_storage.is_available() and sharpened_filepath.startswith('users/'):
            success, thumbnail_path = app_storage.create_thumbnail(sharpened_filepath)
            if not success:
                logger.error(f"Failed to create thumbnail in App Storage: {thumbnail_path}")
                thumbnail_path = None
        else:
            success, result = create_thumbnail(temp_sharpened_filepath if os.path.exists(temp_sharpened_filepath) else sharpened_filepath)
            if not success:
                logger.error(f"Failed to create thumbnail: {result}")
                thumbnail_path = None
            else:
        ...
        # Get sharpened image info
        from photovault.utils.enhanced_file_handler import get_image_info_enhanced
        sharpened_image_info = get_image_info_enhanced(sharpened_filepath)
        
        # Get file size
        file_size = None
        if sharpened_image_info:
            file_size = sharpened_image_info.get('size_bytes')
        if not file_size:
            try:
                if os.path.exists(sharpened_filepath):
                    file_size = os.path.getsize(sharpened_filepath)
                elif os.path.exists(temp_sharpened_filepath):
                    file_size = os.path.getsize(temp_sharpened_filepath)
                else:
                    file_size = getattr(photo, 'file_size', 0)
            except:
                file_size = getattr(photo, 'file_size', 0)
        
        # Create a NEW photo record for the sharpened image
        sharpened_photo = Photo()
        sharpened_photo.user_id = current_user.id
        sharpened_photo.filename = sharpened_filename
        sharpened_photo.original_name = f"Sharpened - {photo.original_name}"
        sharpened_photo.file_path = sharpened_filepath
        sharpened_photo.thumbnail_path = thumbnail_path
        sharpened_photo.file_size = file_size
        sharpened_photo.width = sharpened_image_info.get('width') if sharpened_image_info else getattr(photo, 'width', None)
        sharpened_photo.height = sharpened_image_info.get('height') if sharpened_image_info else getattr(photo, 'height', None)
        sharpened_photo.mime_type = sharpened_image_info.get('format') if sharpened_image_info else getattr(photo, 'mime_type', None)
        sharpened_photo.created_at = datetime.utcnow()
        sharpened_photo.date_taken = getattr(photo, 'date_taken', None)
        sharpened_photo.camera_make = getattr(photo, 'camera_make', None)
        sharpened_photo.camera_model = getattr(photo, 'camera_model', None)
        sharpened_photo.iso = getattr(photo, 'iso', None)
        sharpened_photo.aperture = getattr(photo, 'aperture', None)
        sharpened_photo.shutter_speed = getattr(photo, 'shutter_speed', None)
        sharpened_photo.focal_length = getattr(photo, 'focal_length', None)
        sharpened_photo.flash_used = getattr(photo, 'flash_used', None)
        sharpened_photo.gps_latitude = getattr(photo, 'gps_latitude', None)
        sharpened_photo.gps_longitude = getattr(photo, 'gps_longitude', None)
        sharpened_photo.gps_altitude = getattr(photo, 'gps_altitude', None)
        sharpened_photo.location_name = getattr(photo, 'location_name', None)
        sharpened_photo.orientation = getattr(photo, 'orientation', None)
        sharpened_photo.color_space = getattr(photo, 'color_space', None)
        sharpened_photo.description = getattr(photo, 'description', None)
        sharpened_photo.tags = getattr(photo, 'tags', None)
        sharpened_photo.event_name = getattr(photo, 'event_name', None)
        sharpened_photo.estimated_year = getattr(photo, 'estimated_year', None)
        sharpened_photo.ai_metadata = getattr(photo, 'ai_metadata', None)
        
        # Update original photo with sharpened version info
        ...
        # Commit to database
        try:
            db.session.add(sharpened_photo)
            db.session.commit()
            db.session.refresh(sharpened_photo)
            logger.info(f"Sharpened photo created as new record: {sharpened_photo.id}, filename: {sharpened_photo.filename}")
        except Exception as commit_error:
            logger.error(f"Database commit failed for sharpened photo {photo_id}: {commit_error}")
            db.session.rollback()
            return jsonify({
                'success': False,
                'error': 'Failed to save sharpened photo to database'
            }), 500
        
        logger.info(f"Successfully sharpened photo for user {current_user.id}, photo {photo_id}, new photo ID: {sharpened_photo.id}")
        
        return jsonify({
            'success': True,
            'message': 'Photo sharpened successfully',
            'photo_id': sharpened_photo.id,
            'sharpened_filename': sharpened_filename,
            'thumbnail_url': url_for('gallery.uploaded_file', user_id=current_user.id, filename=thumbnail_filename),
            'sharpened_url': url_for('gallery.uploaded_file', user_id=current_user.id, filename=sharpened_filename),
            'applied_settings': applied_settings
        })
        
    except Exception as e:
        logger.error(f"Error sharpening photo {photo_id}: {str(e)}")
        db.session.rollback()
        return jsonify({
            'success': False,
            'error': f'Failed to sharpen photo: {str(e)}'
        }), 500
